...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/07e24d4f-9eda-4cfe-9492-421af543a600/Untitled.png)
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/24675fac-81db-4e75-8ba8-ef40d9758fa0/Untitled.png)

**[귀납적 등각 예측(Inductive conformal prediction, ICP)]**
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/cfc2b7d2-488b-4bdb-9dd2-7f8f0a5b37c7/Untitled.png)
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/56c00398-2b2a-455c-9ee2-ce8521b411df/Untitled.png)
...
![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/f830d3ff-fb20-4e93-8020-dec9be7a824d/Untitled.png)

그리고 M을 직접 전략을 사용하는 H-단계 예측을 예측하는 순환 신경망이라고 하자. 모든 유의 수준 α ∈ [0, 1]에 대해 ICP 기반 등각 예측 알고리즘으로 얻은 간격(interval)은 최대 α의 오류율을 갖는다. 

즉, 아래 식을 만족하게 된다.

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/c46104b8-08fc-4f91-8d03-48793dce9228/Untitled.png)

(증명은 Vovk[51]에서 ICP의 조건부 타당성과 Boole’s inequality에서 따라나온다.)

알고리즘은 다음과 같다:

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/8f984ebe-3c8a-43a4-b307-a1fcbe331644/Untitled.png)
...
- Dataset
    - 속성이 제어된 합성 데이터
        ...
        >: (수학적으로 표현) ![]([https://latex.codecogs.com/svg.image?y_t](https://latex.codecogs.com/svg.image?y_t)) = from k = 0 to t(a^kx_k+e_t, all k {1,…,T}
        
        where ![]([https://latex.codecogs.com/svg.image?x_t](https://latex.codecogs.com/svg.image?x_t)) ~ ![]([https://latex.codecogs.com/svg.image?N(\\mu&space;_x,&space;\\sigma_x^2)](https://latex.codecogs.com/svg.image?N(%5C%5Cmu&space;_x,&space;%5C%5Csigma_x%5E2))), a = 0.9, ![]([https://latex.codecogs.com/svg.image?e_t&space](https://latex.codecogs.com/svg.image?e_t&space);)~ ![]([https://latex.codecogs.com/svg.image?N(0,\\sigma_t^2)](https://latex.codecogs.com/svg.image?N(0,%5C%5Csigma_t%5E2))), ![]([https://latex.codecogs.com/svg.image?\\sigma&space;_t^2](https://latex.codecogs.com/svg.image?%5C%5Csigma&space;_t%5E2)) = 0.1n for n = {1,…,5}
        
        먼저 시계열의 추세를 결정하는 자기회귀 프로세스와 데이터 세트의 고유한 불확실성을 나
        타내는 노이즈 프로세스의 두 가지 구성요소로 구성된 합성 시계열을 생성합니다.3 길이 T
        의 시계열에 대해 다음과 같이 표현됩니다. 수학적으로 xt ~ N(μx, σ2x), a = 0.9는 메모리 매
        개변수이고 t ~ N(0, σ2t)은 노이즈 프로세스입니다. 5개의 정적 잡음 분산 프로파일 σ2t =
        0.1n, n = {1, . . . , 5}.
...
2개의 노이즈 분산 프로필에 대해 2000개의 훈련 시퀀스(CF-RNN이 이 데이터 세트를 1000개의 실제 훈련 및 1000개의 보정 시퀀스로 분할)에서 모델을 훈련합니다. 저자는 예측 간격(H)에 대해 미래 값들(![]([https://latex.codecogs.com/svg.image?y__{t'&plus;1:t'&plus;H}](https://latex.codecogs.com/svg.image?y__%7Bt%27&plus;1:t%27&plus;H%7D))을 기본 적용률(coverage rate)인 90%(α =0.1)로 예측하는 것을 목표로 한다(여기서 T = 15, H = 5). 불확실성 추정 모델을 위한 RNN 하이퍼파라미터는 공정한 비교를 위해 이전 연구에서 정해진 대로 정했다.
...
![table2.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/4cfc9f2b-90e1-4128-b164-4b6821795ac2/table2.png)

![table3.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/5ffdf337-3c85-4588-8bc0-a67dd60e8b51/table3.png)
...
![figure3.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/8195cc29-fed8-4094-97cb-2ecae4e1225b/figure3.png)
...
#### 현실 시계열 데이터에 대한 실험 결과

CF-RNN, MQ-RNN, DP-RNN 모델들을 아래의 표4에서 처럼 3개의 데이터셋에 대해 훈련시켰다. 

![table4.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/1690c113-0ced-4376-b2e1-ef084753b66b/table4.png)
...
![table5.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/f42a512b-a28a-4bb6-a87b-0a0e638d59f7/table5.png)
...
![table6.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/0f86d162-41ff-4fe8-8996-6cc9fef144a8/table6.png)
