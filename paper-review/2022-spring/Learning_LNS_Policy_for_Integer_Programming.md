# Learning Large Neighborhood Search Policy for Integer Programming Paper Review
...
**Rewards** : $r_t(s_t, a_t)$ = $\mu^T(x_t - x_{t+1})$ objection value인 $\mu^Tx$가 얼마나 변했는지에 대한 것입니다.

**Policy** : GNN기반의 policy network를 가지고, $\pi(a_t|s_t)$는 $s_t$가 given 되었을 때 선택될 variable들의 subset들의 conditional probability distribution을 의미합니다. (ie. 변수 $x_1, x_2$가 있을 때, 주어진 state $s_t$에서 $(x_1), (x_2) ,(x_1, x_2)$가 각각 destroy variable로 선택될 확률을 나타냄)
...
여기서 biparite graph $\mathcal{G} = (\mathcal{V, C},A)$로 구성되어있는데 $\mathcal{V}$는 변수들의 갯수만큼의 노드를 의미하고  각 노드들은 $d_v$ 차원의 노드 features를 가지고 있습니다. $\mathcal{C}$는 제약식의 갯수만큼의 노드를 의미하고 각 노드들은 $d_c$차원의 노드 features를 가지고 있습니다. 마지막으로 A는 노드 $v_i$와 $c_j$간의 weight를 나타내는 edge로 실제 IP문제에서 incidence matrix역할을 합니다. 즉 위의 예시에서는 4개의 변수와 3개의 제약식이 있으므로 $A \in R^{4 \times 3}$ 가 됩니다.

각 노드들의 embedding은 Graph Convolutional Network 구조를 이용하는데

>$C^{(k+1)} = C^{(k)} + \sigma(LN(AV^{(k)}W_v^{(k)})$

>$V^{(k+1)} = V^{(k)} + \sigma(LN(A^TC^{(k)}W_c^{(k)}), k = 0,...,K$

다음과 같이 각 variable node와 constaint node들이 업데이트 됩니다.

여기서 $V^{(k)}, C^{(k)}$는 각각 k번째 layer에서의 variable node들($[v_1^{(k)} \cdots v_n^{(k)}]$), constraint node들($[c_1^{(k)} \cdots c_n^{(k)}]$)를 의미하고, LN, $\sigma(\cdot)$는 layer normalization, tanh함수를 의미합니다.

이렇게 K iteration을 통해 최종 업데이트된 $V^{(k)}$에 해당되는 각 변수에 해당되는 노드들 $[v_1^{(k)} \cdots v_n^{(k)}]$은 마지막에 MLP layer와 sigmoid 활성함수를 통과하여 $\pi^i(a_t^i|s_t)$ 즉 i번째 변수가 선택될 확률이라는 policy와 같은 의미를 갖게 됩니다. 따라서 모든 변수들에 대해 MLP($v_i$)를 통과시킨 뒤 베르누이 샘플링을 통하여 destroy할 subset들을 구성하게 됩니다.
...
만약 문제의 사이즈가 커진다면 action space가 매우 커질 것이고, 이는 즉 Q-network가 매우 sparse할 수 있으므로 Q-network를 바로 학습시키기에는 적절하지 않을 수 있습니다. 논문에서는 actor의 식에서의 $log\pi_\theta(a_t|s_t)$를 위에서 Action factorization에서 언급한대로

$\pi(a_t|s_t) = \prod_{i=1}^n \pi^i(a_t^i|s_t)$를 이용하여 다음과 같이 나타냅니다.

>수정된 Actor_loss : $L(\theta) = E_D[(Q_w(s_{t}, a_{t}) \sum_{i=1}^n log\pi_\theta(a_t|s_t)]$

**clipping & masking**

마지막 테크닉으로 보다 넓은 범위의 action space를 exploration하기 위해서 각 variable이 선택될 확률을 $[\epsilon, 1-\epsilon], \epsilon < 0.5$과 같이 clipping을 합니다. 예를 들어 $\epsilon$ = 0.2라고 하면 모든 variable들의 선택될 확률은 [0.2, 0.8] 범위를 벗어날 수 없는 것입니다.. 이는 매우 높거나 낮게 선택되는 variable들로 인해 학습이 편향되지 않도록 합니다. 또한 sub-IP문제로 적당하지않은 모든 variable들이 선택되는경우나 아닌경우에 대해서는 masking을 통해 그러한 경우를 방지했습니다.
