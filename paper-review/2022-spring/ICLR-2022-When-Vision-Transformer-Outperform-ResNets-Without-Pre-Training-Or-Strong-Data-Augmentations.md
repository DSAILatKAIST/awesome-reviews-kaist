...
- (a) vs (b),(c) : ResNet에 비해서 conv-free한 방법은 loss의 landscape가 sharper함을 확인할 수 있다. Table1을 보면 Hessian 행렬에서 얻은 고윳값 중 제일 큰 &nbsp; ![](https://latex.codecogs.com/gif.latex?\lambda_{max})) $$\lambda_{max}$$을 확인할 수 있다. $\lambda_{max}$는 landscape의 최대(최악) 곡률이다. ResNet과 비교하면 ViT와 특히, Mixer가 상당히 높다는 것을 알 수 있다
